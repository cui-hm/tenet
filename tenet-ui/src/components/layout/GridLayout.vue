...
export default {
  ...
  created() {
    ...
    self.eventBus_.$on('resizeEvent', self.resizeEventHandler)
    self.eventBus_.$on('dragEvent', self.dragEventHandler)
    self.$emit('layout-created', self.layout)
  },
  beforeDestroy: function() {
    //Remove listeners
    this.eventBus_.$off('resizeEvent', this.resizeEventHandler)
    this.eventBus_.$off('dragEvent', this.dragEventHandler)
  ...
  watch: {
    width: function(newval, oldval) {
      ...
      this.$nextTick(function() {
        ...
        this.eventBus_.$emit('updateWidth', this.width)
        if (oldval === null) {
          /*
    ...
    colNum: function(val) {
      this.eventBus_.$emit('setColNum', val)
    },
    rowHeight: function() {
      this.eventBus_.$emit('setRowHeight', this.rowHeight)
    },
    isDraggable: function() {
      this.eventBus_.$emit('setDraggable', this.isDraggable)
    },
    isResizable: function() {
      this.eventBus_.$emit('setResizable', this.isResizable)
    },
    responsive() {
      if (!this.responsive) {
        this.$emit('update:layout', this.originalLayout)
        this.eventBus_.$emit('setColNum', this.colNum)
      }
      this.onWindowResize()
    },
    maxRows: function() {
      this.eventBus_.$emit('setMaxRows', this.maxRows)
    },
    isDragging(newVal) {
      if(newVal === true) {
        this.$emit('layout-update-start')
      }
    }
  },
  methods: {
    ...
    onWindowResize: function() {
      ...
      this.eventBus_.$emit('resizeEvent')
    },
